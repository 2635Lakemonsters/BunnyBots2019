...
public class Path2D {
    ...
    public Vector2 getTangent(double time) {
        ...
        rValue = rValue.scale(flipTangent);
        return rValue;
    }

    public Vector2 getRobotDirection(double time) {
        return getTangent(time)
            .rotateBy(Rotation2.fromDegrees(-m_headingCurve.getValue(time)));
    }
    ...
    public Vector2 getPositionAtEase(double ease) {
        ...
        if (isMirrored())
            rValue = new Vector2(-rValue.x, rValue.y);
        return rValue;
    }
    ...
    public Vector2 getTangentAtEase(double ease) {
        ...
        if (isMirrored())new Vector2(-rValue.x, rValue.y);
            rValue = new Vector2(-rValue.x, rValue.y);
        return rValue;
    }
    ...
    public Vector2 getSidePosition(double time, double xOffset) {  // offset can be positive or negative (half the width of the robot)
        ...
        Vector2 tangent = getTangent(time)
                ...
                .perpendicular()
                .scale(xOffset)
                .scale(Math.copySign(1.0, speed));
        return centerPosition.add(tangent);
    }
    ...
    public double getAccelerationAtEase(double ease) {
        ...
        Vector2 delta = tangent2.subtract(tangent1);
        return delta.length * Path2DPoint.STEPS / deltaEase / getDuration() / getDuration();  // this is how much it would curve over the entire path length
    }

    public Vector2 getVelocityAtEase(double ease) {
        Vector2 velocity = getTangentAtEase(ease);
        velocity = velocity.scale(Path2DPoint.STEPS);
        return velocity;
    }
    ...
    public Vector2 getVelocityAtTime(double time) {
        Vector2 tangent = getTangent(time);
        tangent.normal();
        Vector2 velocity = tangent.scale( m_easeCurve.getDerivative(time) * m_xyCurve.getLength());
        return velocity;
    }
