...
import static frc.robot.model.MotionKey.SlopeMethod.SLOPE_PLATEAU;

import org.frcteam2910.common.math.Vector2;

import static frc.robot.model.MotionKey.SlopeMethod.SLOPE_MANUAL;
...
public class MotionKey {
    ...
    public void setTime(double time) {
        m_timeAndValue = new Vector2(time, m_timeAndValue.y);
        onPositionChanged();
    }
    ...
    public void setValue(double value) {
        m_timeAndValue = new Vector2(m_timeAndValue.x, value);
        onPositionChanged();
    }
    ...
    private void calculateTangents() {
        ...
        switch (getPrevSlopeMethod()) {
            case SLOPE_MANUAL:
                m_prevTangent = new Vector2(Math.cos(getPrevAngleAndMagnitude().x), Math.sin(getPrevAngleAndMagnitude().x));
                if (m_prevKey != null)
                    //Vector2.Companion.multiply(m_prevTangent, getTimeAndValue().x - m_prevKey.getTimeAndValue().x);
                    m_prevTangent.scale(getTimeAndValue().x - m_prevKey.getTimeAndValue().x);
                break;
            case SLOPE_LINEAR:
                if (m_prevKey != null)
                    //m_prevTangent = Vector2.Companion.subtract(getTimeAndValue(), m_prevKey.getTimeAndValue());
                    m_prevTangent = getTimeAndValue().subtract(m_prevKey.getTimeAndValue());
                break;
            case SLOPE_FLAT:
                if (m_prevKey != null)
                    m_prevTangent = new Vector2((getTimeAndValue().x - m_prevKey.getTimeAndValue().x) * 0.5, 0.0);
                break;
            case SLOPE_SMOOTH:
            ...
            case SLOPE_CLAMPED: {
                ...
                if (m_prevKey != null && Math.abs(m_prevKey.getValue() - getValue()) <= fClampTolerence) // make Flat
                    m_prevTangent = new Vector2(getTime() - m_prevKey.getTime(), 0.0f);
                else if (m_nextKey != null && Math.abs(m_nextKey.getValue() - getValue()) <= fClampTolerence) // Make Flat
                {
                    if (m_prevKey != null)
                        m_prevTangent = new Vector2(getTime() - m_prevKey.getTime(), 0.0f);
                    else
                        m_prevTangent = new Vector2(0.0f, 0.0f);
                } else
                    bCalcSmoothPrev = true;
                break;
            }
            case SLOPE_PLATEAU:
                if (m_prevKey == null || m_nextKey == null) {
                    if (m_prevKey != null)
                        m_prevTangent = new Vector2(getTime() - m_prevKey.getTime(), 0.0f); // Make Flat
                    else
                        m_prevTangent = new Vector2(0.0f, 0.0f);
                } else // we have a prev and a next, lets see if both the prev's out tangent and the next's in tangent are both either greater or less than our value, if so lets make out tangent flat
                {
                    ...
                    if (m_prevKey.getNextSlopeMethod() == SLOPE_PLATEAU)
                        ...
                        Vector2 vPrevTangent = vPrevPos.add(m_prevKey.getNextTangent().scale(1.0 / 3.0));
                    ...
                    if (m_nextKey.getPrevSlopeMethod() == SLOPE_PLATEAU)
                        ...
                        Vector2 vNextTangent = vNextPos.subtract(m_nextKey.getPrevTangent().scale(1.0 / 3.0));
        ...
        switch (getNextSlopeMethod()) {
            case SLOPE_MANUAL:
                m_nextTangent = new Vector2(Math.cos(getNextAngleAndMagnitude().x), Math.sin(getNextAngleAndMagnitude().x));
                if (m_nextKey != null)
                    //Vector2.Companion.multiply(m_nextTangent, m_nextKey.getTimeAndValue().x - getTimeAndValue().x);
                    m_nextTangent.scale(m_nextKey.getTimeAndValue().x - getTimeAndValue().x);
                break;
            case SLOPE_LINEAR:
                if (m_nextKey != null)
                    //m_nextTangent = Vector2.Companion.subtract(m_nextKey.getTimeAndValue(), getTimeAndValue());
                    m_nextTangent = m_nextKey.getTimeAndValue().subtract(getTimeAndValue());
                break;
            case SLOPE_FLAT:
                if (m_nextKey != null)
                    m_nextTangent = new Vector2(m_nextKey.getTimeAndValue().x - getTimeAndValue().x, 0.0f);
                break;
            case SLOPE_SMOOTH:
            ...
            case SLOPE_CLAMPED: {
                ...
                {
                    if (m_nextKey != null)
                        m_nextTangent = new Vector2(m_nextKey.getTime() - getTime(), 0.0f);
                    else
                        m_nextTangent = new Vector2(0.0f, 0.0f);
                } else if (m_nextKey != null && Math.abs(m_nextKey.getValue() - getValue()) <= fClampTolerence) // Make Flat
                    m_nextTangent = new Vector2(m_nextKey.getTime() - getTime(), 0.0f);
                else
                    bCalcSmoothNext = true;
            ...
            case SLOPE_PLATEAU:
                if (m_prevKey == null || m_nextKey == null) {
                    if (m_nextKey != null)
                        m_nextTangent = new Vector2(m_nextKey.getTime() - getTime(), 0.0f); // Make it flat
                    else
                        m_nextTangent = new Vector2(0.0f, 0.0f);
                } else // we have a prev and a next, lets see if both the prev's out tangent and the next's in tangent are both either greater or less than our value, if so lets make out tangent flat
                {
        ...
        if (bCalcSmoothPrev || bCalcSmoothNext) {
            if (m_prevKey != null && m_nextKey != null) {
                ...
                Vector2 delta = m_nextKey.getTimeAndValue().subtract(m_prevKey.getTimeAndValue());
                ...
                if (m_nextKey != null) {
                    if (bCalcSmoothPrev)
                        ...
                        m_prevTangent = m_nextKey.getTimeAndValue().subtract(getTimeAndValue());
    ...
    public void setMagnitude(double magnitude) {
        m_prevAngleAndMagnitude = new Vector2(m_prevAngleAndMagnitude.x, magnitude);
        m_nextAngleAndMagnitude = new Vector2(m_prevAngleAndMagnitude.x, magnitude);
    ...
    public void setAngle(double angle) {
        m_prevAngleAndMagnitude = new Vector2(angle, m_prevAngleAndMagnitude.y);
        m_nextAngleAndMagnitude = new Vector2(angle, m_prevAngleAndMagnitude.y);
        m_nextSlopeMethod = m_prevSlopeMethod = SLOPE_MANUAL;
        onPositionChanged();
    }
